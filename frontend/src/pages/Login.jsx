...
import { Shield, Lock, User, ArrowRight, Zap, Globe, Loader2, AlertCircle } from 'lucide-react';
...
const Login = () => {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      // Save user to local storage if needed, or just rely on cookies
      ...
      <div className="w-full max-w-[440px] z-10">
        <div className="flex flex-col items-center mb-12">
          ...
          <div className="flex items-center gap-3 mt-2">
            ...
            <span className="text-[10px] text-slate-500 font-bold uppercase tracking-[0.3em]">Command Hub Authentication</span>
          </div>
        </div>
        ...
        <div className="glass-card p-10 rounded-[40px] border border-white/5 relative overflow-hidden group">
          ...
          <h2 className="text-xl font-black italic text-white uppercase tracking-tight mb-8">Agent Access</h2>
          ...
          <form className="space-y-6" onSubmit={handleLogin}>
            <div className="space-y-2">
              <label className="text-[10px] font-black text-slate-500 uppercase tracking-[0.2em] flex items-center gap-2">
                <User className="w-3.5 h-3.5" /> Identity Identifier
              </label>
              <input 
                type="email" 
                required
                ...
                className="w-full bg-white/5 border border-white/10 rounded-2xl px-6 py-4 text-sm focus:outline-none focus:border-primary/50 focus:bg-white/10 transition-all text-white placeholder:text-slate-700 font-bold"
              />
            </div>
          ...
          <div className="mt-8 text-center">
            <Link to="/register" className="text-[10px] font-black text-slate-500 hover:text-primary uppercase tracking-widest transition-colors">
              New Agent? Enroll in Command Hub
            </Link>
          </div>
          ...
          <div className="mt-10 flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <span className="text-[9px] font-bold text-slate-500 uppercase tracking-widest">Quantum Encryption</span>
            </div>
            <div className="flex items-center gap-2">
              <div className="w-1.5 h-1.5 rounded-full bg-success"></div>
              <span className="text-[9px] font-bold text-slate-500 uppercase tracking-widest">Node: DC-04</span>
            </div>
          </div>
        </div>
        
        <p className="text-center mt-10 text-[10px] font-bold text-slate-600 uppercase tracking-[0.1em] leading-relaxed">
          Authorized personnel only. <br/> Access attempts are logged and scrutinized.
        </p>
      </div>
    </div>
  );
